...
const emptyZone: ZoneData = { text: '', image: null };
const initialZonePrompts: ZonePrompts = { 
    red: { ...emptyZone }, 
    yellow: { ...emptyZone }, 
    green: { ...emptyZone }, 
    blue: { ...emptyZone }, 
...
export default function App() {
  ...
  const getEffectiveReferences = (basePool: ImageFile[]) => {
      ...
      // NEW: Inject Zone-specific images into the pool
      // Fix: Cast Object.values result to ZoneData[] to avoid "unknown" type access errors
      (Object.values(zonePrompts) as ZoneData[]).forEach(zone => {
          if (zone.image) allRefs.push(zone.image);
      });
  ...
  const constructFullPrompt = useCallback((
    ...
    // Determine starting index for references in Gemini parts array
    // Input 0: Target, Input 1: Mask (if present)
    let currentInputIdx = 1 + (mode === 'edit' || (mode === 'style' && avatarImage?.maskDataUrl) ? 1 : 0);
    
    // Skip base reference pool images indices
    currentInputIdx += basePoolSize;

    // Add Identity indices
    if (iLock.enabled) {
        if (structureImage) currentInputIdx++;
        if (textureImage) currentInputIdx++;
    }
    // Add Garment indices
    if (gLock.enabled) currentInputIdx += garmentImages.length;
    // Add Pose/Scene indices
    if (pLock.enabled && poseLockImage) currentInputIdx++;
    if (sLock.enabled && sceneLockImage) currentInputIdx++;
    // Add Advanced indices
    if (isAdvancedUploads) {
        (Object.values(advancedRefs) as AdvancedReferenceBucket[]).forEach(bucket => {
            if (bucket.enabled) currentInputIdx += bucket.images.length;
        });
    }

    if (mode === 'edit' && editSubMode === 'mask') {
        const zoneParts = [];
        const zoneKeys: Array<keyof ZonePrompts> = ['red', 'yellow', 'green', 'blue', 'purple'];
        
        zoneKeys.forEach(k => {
            const z = zones[k];
            if (z.text || z.image) {
                let instruction = `[${k.toUpperCase()} ZONE]: ${z.text || 'Apply style from associated reference'}`;
                if (z.image) {
                    instruction += ` (ASSOCIATED REFERENCE: Input ${currentInputIdx})`;
                    currentInputIdx++;
                }
                zoneParts.push(instruction);
            }
        });
    ...
    return parts.join('\n\n');
  }, [mode, editSubMode, aspectRatio, avatarImage, structureImage, textureImage, garmentImages, poseLockImage, sceneLockImage, isAdvancedUploads, advancedRefs]);
  ...
  const handleStyleTransfer = useCallback(async () => {
    ...
    const basePool = referenceImages;
  ...
  const handleInpaintingEdit = useCallback(async () => {
    ...
    const basePool = editReferenceImages;
    const finalPrompt = constructFullPrompt(prompt, promptConfig, zonePrompts, identityConfig, garmentConfig, poseLockConfig, sceneLockConfig, realismConfig, basePool.length);
    const seed = curationSeed ? parseInt(curationSeed) : Math.floor(Math.random() * 2147483647);
  ...
  const handleZonePromptChange = (zone: keyof ZonePrompts, val: string) => {
      setZonePrompts(prev => ({ ...prev, [zone]: { ...prev[zone], text: val } }));
  };

  const handleZoneImageChange = (zone: keyof ZonePrompts, img: ImageFile | null) => {
      setZonePrompts(prev => ({ ...prev, [zone]: { ...prev[zone], image: img } }));
  };
                ...
                <div className="bg-light-card dark:bg-dark-card rounded-md border-[0.5px] border-dark-text/10 dark:border-white/10 p-5 md:p-10 space-y-8 md:space-y-10">
                    ...
                    <div className="relative pt-4">
                        ...
                        <PromptInput 
                            ...
                            onZonePromptChange={handleZonePromptChange}
                            onZoneImageChange={handleZoneImageChange}
                        />
